...
VS_OUT main(VS_IN vin)
{
	...
	float temp_output_60_0 = ((wind_speed*0.05) * (something.triple_speed_iTime));//三倍速と風の速度を計算
	float2 appendResult249 = (float2(wind_direction.x, wind_direction.z));
	float3 wind_vectors_99 = wind_vectors.SampleLevel(anisotropic_sampler_state, (wind_amplitude_multiplier * wind_amplitude * (ase_worldPos.xz * 0.01) + temp_output_60_0 * appendResult249), 0.0);
	wind_vectors_99 = wind_vectors_99 * 2.0 - 1.0;//-1~1
	...
	float3 wind17 = (((wind_vectors_99 *  vin.color.g) * max_wind_strength * wind_strength) + appendResult283);
...
//#else //aselc
	float4 ase_lightColor = LightColor0;
//#endif //aselc
	vout.vertexToFrag332 = (((max(dotResult36, 0.0) * vin.color.b) * TransmissionColor.a) * (TransmissionColor * ase_lightColor));
