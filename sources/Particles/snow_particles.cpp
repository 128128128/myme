...
snow_particles::snow_particles(ID3D11Device* device, DirectX::XMFLOAT3 initial_position)
{
	...
	buffer_desc.ByteWidth = static_cast<UINT>(sizeof(snow_particle) * snow_particle_count);
	buffer_desc.StructureByteStride = sizeof(snow_particle);
	...
	unordered_access_view_desc.Buffer.NumElements = static_cast<UINT>(snow_particle_count);
	...
	shader_resource_view_desc.Buffer.NumElements = static_cast<UINT>(snow_particle_count);
	...
	create_vs_from_cso(device, "shader//snowfall_particles_vs.cso", vertex_shader.ReleaseAndGetAddressOf(), nullptr, nullptr, 0);
	create_ps_from_cso(device, "shader//snowfall_particles_ps.cso", pixel_shader.ReleaseAndGetAddressOf());
	create_gs_from_cso(device, "shader//snowfall_particles_gs.cso", geometry_shader.ReleaseAndGetAddressOf());
	create_cs_from_cso(device, "shader//snowfall_particles_cs.cso", compute_shader.ReleaseAndGetAddressOf());
...
void snow_particles::render(ID3D11DeviceContext* immediate_context)
{
	...
	immediate_context->Draw(static_cast<UINT>(snow_particle_count), 0);
