...
noise_map::noise_map(ID3D11Device* device, const char* filename, bool force_srgb, bool enable_caching, D3D11_FILTER sampler_filter, D3D11_TEXTURE_ADDRESS_MODE sampler_texture_address_mode, DirectX::XMFLOAT4 sampler_boarder_color)
{
	HRESULT hr{ S_OK };
	...
	buffer_desc.ByteWidth = sizeof(cb_noise);
	buffer_desc.Usage = D3D11_USAGE_DYNAMIC;
	buffer_desc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
	buffer_desc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	buffer_desc.MiscFlags = 0;
	buffer_desc.StructureByteStride = 0;
	subresource_data.pSysMem = &cb_noise;
	subresource_data.SysMemPitch = 0;
	subresource_data.SysMemSlicePitch = 0;
	hr = device->CreateBuffer(&buffer_desc, &subresource_data, noise_buffer.GetAddressOf());
	_ASSERT_EXPR(SUCCEEDED(hr), hr_trace(hr));
	...
	//shader create
	create_vs_from_cso(device, "shader//noise_create_vs.cso", vertex_shader.GetAddressOf(), input_layout.GetAddressOf(), input_element_desc, _countof(input_element_desc));
	create_ps_from_cso(device, "shader//noise_create_ps.cso", pixel_shader.GetAddressOf());
...
void noise_map::render(ID3D11DeviceContext* immediate_context,
	...
	// r, g, b : color
{
	...
	hr = immediate_context->Map(noise_buffer.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &mapped_subresource1);
	_ASSERT_EXPR(SUCCEEDED(hr), hr_trace(hr));

	noise* cb_noise{ reinterpret_cast<noise*>(mapped_subresource1.pData) };
	if (cb_noise != nullptr)
	{
		cb_noise->kind_of_noise = num_noise;
		cb_noise->octaves = sum_octaves;
		cb_noise->amplitude = num_amplitude;
		cb_noise->param = n_param;
		cb_noise->mat0 = mat0_param;
		cb_noise->mat1 = mat1_param;
	}

	immediate_context->Unmap(noise_buffer.Get(), 0);

	UINT stride{ sizeof(vertex) };
	UINT offset{ 0 };
	immediate_context->IASetVertexBuffers(0, 1, vertex_buffer.GetAddressOf(), &stride, &offset);
	immediate_context->PSSetConstantBuffers(4, 1, noise_buffer.GetAddressOf());
	immediate_context->VSSetConstantBuffers(4, 1, noise_buffer.GetAddressOf());
...
void noise_map::render(ID3D11DeviceContext* immediate_context, float dx, float dy, float dw, float dh)
{
...
void noise_map::textout(ID3D11DeviceContext* immediate_context, std::string s, float x, float y, float w, float h, float r, float g, float b, float a)
{
...
void noise_map::DebugDrawGUI()
{
	...
	{
		if (ImGui::CollapsingHeader("Noise Pattern", ImGuiTreeNodeFlags_DefaultOpen))
		{
			ImGui::Text("0...fractal , 1...value , 2...perlin");

			ImGui::InputFloat("kind of noise", &num_noise, 1.0f, 2.0f);
			if (num_noise == 0) {
				ImGui::InputFloat("octaves", &sum_octaves, 1.0f, 20.0f);
				ImGui::SliderFloat("amplitude", &num_amplitude, 0.0f, 1.0f);
				ImGui::SliderFloat2("mat0", &mat0_param.x, 1.0f, 5.0f);
				ImGui::SliderFloat2("mat1", &mat1_param.x, 1.0f, 5.0f);
			}
			else
				ImGui::SliderFloat("param", &n_param, 1.0f, 50.0f);
