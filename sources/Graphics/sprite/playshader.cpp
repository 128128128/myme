...
playshader::playshader(ID3D11Device* device, const char* filename, bool force_srgb, bool enable_caching, D3D11_FILTER sampler_filter, D3D11_TEXTURE_ADDRESS_MODE sampler_texture_address_mode, DirectX::XMFLOAT4 sampler_boarder_color)
{
	HRESULT hr{ S_OK };
	...
	buffer_desc.ByteWidth = sizeof(forplays);
	buffer_desc.Usage = D3D11_USAGE_DYNAMIC;
	buffer_desc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
	buffer_desc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	buffer_desc.MiscFlags = 0;
	buffer_desc.StructureByteStride = 0;
	subresource_data.pSysMem = &forplays;
	subresource_data.SysMemPitch = 0;
	subresource_data.SysMemSlicePitch = 0;
	hr = device->CreateBuffer(&buffer_desc, &subresource_data, forplay_buffer.GetAddressOf());
	_ASSERT_EXPR(SUCCEEDED(hr), hr_trace(hr));
	...
	//shader create
	create_vs_from_cso(device, "shader//playshader_vs.cso", vertex_shader.GetAddressOf(), input_layout.GetAddressOf(), input_element_desc, _countof(input_element_desc));
	create_ps_from_cso(device, "shader//playshader_ps.cso", pixel_shader.GetAddressOf());
...
void playshader::render(ID3D11DeviceContext* immediate_context,
	...
	// r, g, b : color
{
	...
	hr = immediate_context->Map(forplay_buffer.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &mapped_subresource1);
	_ASSERT_EXPR(SUCCEEDED(hr), hr_trace(hr));

	forplay* forplays{ reinterpret_cast<forplay*>(mapped_subresource1.pData) };
	if (forplays != nullptr)
	{
		Mouse& m = Input::Instance().GetMouse();
		
		forplays->mouse.x= m.GetPositionX();
		forplays->mouse.y= m.GetPositionY();

	    forplays->mouse.z = m.GetButton() & Mouse::BTN_LEFT;
		
	    forplays->mouse.w = m.GetButton() & Mouse::BTN_RIGHT;

		forplays->coverage = cover;
...
void playshader::render(ID3D11DeviceContext* immediate_context, float dx, float dy, float dw, float dh)
{
...
void playshader::textout(ID3D11DeviceContext* immediate_context, std::string s, float x, float y, float w, float h, float r, float g, float b, float a)
{
