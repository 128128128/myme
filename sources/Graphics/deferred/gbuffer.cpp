...
struct dir_light_param {
	DirectX::XMFLOAT4 dir_light_direction;  //direction(方向) 
	DirectX::XMFLOAT4 dir_light_color; //color(カラー)
};
Microsoft::WRL::ComPtr<ID3D11Buffer> dir_light_buffer;

int num_dir_light = 0;
dir_light_param dir_light[100];

//light initialize function
void light_initialize(ID3D11Device* device)
{
	// create constant buffer
	D3D11_BUFFER_DESC bdLight;
	ZeroMemory(&bdLight, sizeof(D3D11_BUFFER_DESC));
	bdLight.ByteWidth = sizeof(dir_light_param);
	bdLight.Usage = D3D11_USAGE_DEFAULT;
	bdLight.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
	bdLight.CPUAccessFlags = 0;
	device->CreateBuffer(&bdLight, NULL, dir_light_buffer.GetAddressOf());
}

//add light function
//direction.xyz+color.rgb
void light_add(DirectX::XMFLOAT3 dir, float r, float g, float b)
{
	dir_light[num_dir_light].dir_light_direction =
	{ dir.x, dir.y, dir.z, 0.0f };
	dir_light[num_dir_light].dir_light_color = { r,g,b,1.0f };
	num_dir_light++;
}

//add point light function
// posx, posy, posz, distance(減衰距離)
void light_add_point_light(
	DirectX::XMFLOAT3 pos, float distance,
...
//setting light
//ID3D11DeviceContext, light number
void LIGHT_Set(ID3D11DeviceContext* immediate_context,int index)
{
	immediate_context->UpdateSubresource(
		dir_light_buffer.Get(), 0, NULL,
		&dir_light[index], 0, 0);
	immediate_context->PSSetConstantBuffers(1, 1, dir_light_buffer.GetAddressOf());
}

void gbuffer::initialize(ID3D11Device* device, DirectX::XMFLOAT3 dir)
{
	light_initialize(device);

	light_add(dir, 1.0f, 1.0f, 1.0f);
	/*DirectX::XMFLOAT3 pos = {};
	for (int i = 0; i < 2; i++) {
		pos = { 2.0f,
			2.0f,
			2.0f };
		light_ad_point_light(
			pos,
			8,
			1.0f,
			1.0f,
	...
	create_ps_from_cso(device, "shader//dir_light_ps.cso", dir_light_ps.GetAddressOf());
...
void gbuffer::rendering(ID3D11DeviceContext* immediate_context)
{
	...
	// Slot14
	immediate_context->PSSetShaderResources(
		7, 1, srv_shadow_param.GetAddressOf());
	...
	immediate_context->DrawIndexed(4, 0, 0);
	//for (int i = 1; i < NumDirLight; i++) {
	//	light_set(immediate_context,i);
	//	immediate_context->DrawIndexed(4, 0, 0);
	//}
...
void gbuffer::set_color_resource(ID3D11DeviceContext* immediate_context, int slot)
{
	immediate_context->PSSetShaderResources(
